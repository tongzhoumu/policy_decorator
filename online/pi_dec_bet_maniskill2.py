...
def parse_args():
    ...
    parser.add_argument("--base-policy-ckpt", type=str, default='checkpoints/bet_StackCube/checkpoints/best.pt')
    ...
    parser.add_argument("--res-scale", type=float, default=0.2) # alpha, need to tune
    parser.add_argument("--prog-explore", type=int, default=2_000_000) # H, need to tune
...
def evaluate(n, base_policy, res_actor, eval_envs, device):
    ...
    while len(result['return']) < n:
        ...
        with torch.no_grad():
            base_actions_tensor = base_policy.get_eval_action(obs_seq_tensor).detach()
            base_actions = base_actions_tensor.cpu().numpy()
            actor_input = obs_seq_tensor[:, -1] if args.actor_input == 'obs' else torch.cat([obs_seq_tensor[:, -1], base_actions_tensor], dim=1)
        ...
        obs_seq, rew, terminated, truncated, info = eval_envs.step(final_actions)
...
if __name__ == "__main__":
    ...
    assert len(envs.single_observation_space.shape) == 2 # (context_window, obs_dim)
    ...
    for key in ['ema_agent', 'agent', 'actor', 'q']:
        if key in checkpoint:
            ...
            break
    # base_policy.eval()
    base_policy.train() # BeT performs better with dropout in our experiments
    ...
    if args.critic_input == 'concat':
        dummy_env.single_action_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(act_dim * 2,))
    ...
    rb = ReplayBuffer(
        ...
        envs.single_action_space if args.critic_input == 'res' and args.actor_input == 'obs' else gym.spaces.Box(low=-np.inf, high=np.inf, shape=(act_dim * 3,)),
        device,
    ...
    while global_step < args.total_timesteps:
        ...
        for local_step in range(args.training_freq // args.num_envs):
            ...
            obs_seq_tensor = torch.Tensor(obs_seq).to(device) # (num_envs, context_window, obs_dim)
            base_actions_tensor = base_policy.get_eval_action(obs_seq_tensor)
            base_actions = base_actions_tensor.cpu().numpy()
            ...
            if not learning_has_started:
                res_actions = np.array([envs.single_action_space.sample() for _ in range(envs.num_envs)])
            ...
            next_obs_seq, rewards, terminations, truncations, infos = envs.step(final_actions)
            ...
            if args.critic_input == 'res' and args.actor_input == 'obs':
                ...
                base_next_actions = base_policy.get_eval_action(torch.Tensor(real_next_obs_seq).to(device)).cpu().numpy()
        ...
        for local_update in range(num_updates_per_training):
            ...
            if args.critic_input != 'res' or args.actor_input == 'obs_base_action':
                ...
                base_actions = data.actions[:, act_dim:act_dim*2]
                base_next_actions = data.actions[:, -act_dim:]
            else:
